...
class ARButton {
  constructor(renderer, options) {
    ...
    if ("xr" in navigator) {
      ...
      navigator.xr.isSessionSupported("immersive-ar").then((supported) => {
        supported
          ? this.showStartAR(button, button2)
          : this.showARNotSupported(button, button2);
      });
  ...
  showStartAR(button, button2) {
    let currentSession = null;
    const self = this;

    this.stylizeElement(button, true, 30, true);
    this.stylizeElement(button2, true, 30, true);
    ...
    function onSessionStarted(session) {
      ...
      self.stylizeElement(button2, false, 12, true);
